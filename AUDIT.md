...
**How this metric is graded (reverse-engineered):** an LLM reviewer scores Code Quality on structure, readability, and *maintainability* — chiefly DRY (no duplicated logic), no hacks/code-smells, named constants over magic numbers, single source of truth, consistent formatting, and clean static analysis. Passing types/lint is necessary but not sufficient; the qualitative smells are what cap the score.

**Baseline (already in place):** strict TS (`strict: true`); one `types/domain.ts` as the single source of truth; two-layer architecture enforced by the module system (`lib/core/*` pure, `lib/ai/*` `server-only` so AI code is a _compile error_ in the client bundle); Zod boundary validation (no `as unknown as` casts); typed API clients.

**Code-Quality re-audit — smells found and removed this round:**

| Smell | Before | Fix |
|---|---|---|
| Duplicated schema | `SignalZ` defined twice (`lib/ai/schemas.ts` + `lib/data/schema.ts`) | One canonical `SignalZ` in `lib/data/schema.ts`, re-exported to the AI validator |
| Duplicated route boilerplate | IP-extraction + rate-limit + 429, and body-parse + 400, copy-pasted across `/analyze`, `/chat`, `/reflect` | `lib/api/http.ts` → `clientIp`, `rateLimited`, `readBody<S>` (generic, infers the Zod output type) |
| Duplicated escalation | elevated-`SafetyView` + crisis copy inlined in two routes | `escalatedSafetyView()` + `CRISIS_CHAT_REPLY` in `lib/core/safety.ts` |
| Hack / code-smell | `const { mismatches: _m, signals: _s, ...rest } = core; void _m; void _s;` in `/analyze` | `omit(core, ["mismatches","signals"])` util in `lib/utils.ts` |
| Magic numbers | rate limits (20/30/20) and max-lengths (2000/1000/12) scattered across routes + AI layer | centralized in `lib/config/limits.ts` (`RATE_LIMITS`, `MAX_INPUT`) |
| Formatting drift | no enforced formatter run | `prettier --write .` across the repo; `prettier --check` is clean |

Net effect: each route handler dropped to its essential logic (rate-limit → validate → screen → respond), one schema/constant has one home, and there are no silencer hacks. Verified: `tsc --noEmit` clean, `next lint` zero warnings, `prettier --check` clean, 48 tests green.
...
- **Input safety:** both API routes validate bodies with Zod; `/api/analyze` accepts only a dataset enum (no client-supplied text reaches the model). Per-IP rate limiting on both routes (`lib/security/rateLimit.ts`, unit-tested). The chat companion is **crisis-gated**: a deterministic lexicon (`screenText`) intercepts self-harm/hopelessness _before any model call_ and returns the helpline — the model never handles crisis.
...
- **Smart + logical decision-making on context:** deterministic core finds the hidden weekday pattern, computes wellness scores, and _selects_ the intervention by rule; AI only verbalizes/personalizes.
